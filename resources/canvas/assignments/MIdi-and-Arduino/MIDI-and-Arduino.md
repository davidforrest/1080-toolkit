...
1. **Introduction to MIDI** (Music Instrument Digital Interface)
  ...
  - Introduce simple MIDI input and output circuits for MIDI using Arduino
  - Make a simple MIDI controller with your Arduino
  - Test/play with your instrument and control an external MIDI-enabled device (for this lab, a software synthesizer running on your computer)
...
- Resistors
- MIDI connector jack and MIDI-USB interface
- Your computer - to run a software synthesizer, specifically “VCV Rack”
...
In this lab, we will be wiring up a MIDI controller to transmit information about what sounds we want played (our “controller”). The MIDI controller functions as a MIDI-OUT that will send MIDI data. This MIDI data will then be transferred via a MIDI cable to your software synthesizer, which receives and understands the MIDI data (your “synthesizer” - MIDI-IN) and will play the sounds that were sent.

In your setup, the Arduino MKRZero and buttons function as the MIDI controller. The Arduino MKRZero generates MIDI commands that are sent through the MIDI-USB cable into a port on your computer. These MIDI commands are then interpreted by the VCV Rack software to synthesize sound.
...
1. Wire up the circuit as shown in Fig1 for the **MIDI controller**:

NOTE: The diagram below shows the MIDI connector jack facing toward you, HOWEVER, in your circuit, it **should be facing away from you!** This allows you to easily make connections between the breadboard and the three relevant pins on the back of the MIDI connector. You can see the correct configuration in the circuit photos that follow.
...
- Code files are on Canvas, in the folder [Lab 8 Code](https://canvas.harvard.edu/courses/75224/files/folder/Lab%208%20Code). You'll want the file called `lab8_code.ino`.
- Connect your Arduino MKRZero to your computer and upload the Arduino code to the Arduino.
- Close the Arduino IDE, and disconnect the Arduino board.

## Part 3A: (Install Rack and set up your Audio Output)

You'll need software on your computer to read and interpret the MIDI commands that you are generating from your Arduino instrument. To use VCV Rack as the MIDI synthesizer, you'll first need to install the software. Then we’ll configure the port on our computer to be able to receive MIDI commands.
...
Just to make sure Rack is working, we’re going to try making some sound using your computer’s keyboard as a temporary MIDI controller.
...
You can keep the default settings for all the other Rack modules for now.
...
Now, you should be able to press the push buttons on your breadboard and hear sound being played by the software synthesizer! You should notice the LED on the MIDI-USB connector blinking when MIDI commands are being transmitted, and the yellow LED on the MKRZero should also remain lit while MIDI commands are being transmitted.
...
If everything works as designed/expected, great! You’ve just finished the MIDI part of Lab 8.
...
Hint: we have included an updated code sketch that you can feel free to use, in the [Lab 8 Code](https://canvas.harvard.edu/courses/75224/files/folder/Lab%208%20Code) folder on Canvas.

You can go in two directions with this section of the lab. Either you can expand the Arduino Sound Synthesizer you begin last week, or you can expand the Arduino MIDI controller you've build this week.

- If you build the 14-key Arduino synthesizer, which would use your physical speaker from last week, be sure to use the Arduino sketch `lab8_arduino_synth_14keys.ino`. The speaker should now be connected to digital pin 7, and the buttons should be connected to the analog pins A0-A6, and D0-D6 (14 pins on the arduino in total).
- If you instead build the Arduino 14-key Arduino MIDI controller and use the software synthesizer introduced in this week’s lab, be sure to use the Arduino sketch `lab8_arduino_instrument_14keys.ino`. The buttons should be connected to the analog pins A0-A6, and D0-D6 (14 pins on the arduino in total).
...
- The same goes for code -- be sure to save a separate copy before you begin modifying things, and leave comments throughout so that you can easily remember what your code does!
